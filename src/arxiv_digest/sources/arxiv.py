...
def fetch_todays_papers() -> list[Paper]:
    ...
    for category in config.ARXIV_CATEGORIES:
        ...
        for result in results:
            # 投稿日が今日でないものは除外（最新バッチのみ対象）
            submitted_date = result.published
            if submitted_date is not None:
                # published は datetime (aware/naive 混在の可能性あり)
                if hasattr(submitted_date, "date"):
                    submitted_day = submitted_date.date()
                else:
                    submitted_day = submitted_date
                if submitted_day != today:
                    # 新着ソートなので今日より古いものが来たら以降はスキップ
                    logger.debug(
                        "投稿日 %s が今日 (%s) と異なるためスキップ", submitted_day, today
                    )
                    continue
